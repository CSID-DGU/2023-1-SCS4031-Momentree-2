...
while len(recommended) < 5:
    top_similarities = user_similarities.stack().nlargest(1)  # 유사도가 가장 높은 항목을 추출
    print(top_similarities)
    if top_similarities.index[0][1] not in recommended:  # 이미 추천된 항목이 아니라면
        recommended.append(top_similarities.index[0][1])  # 추천 목록에 추가
